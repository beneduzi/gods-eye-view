...
export function cloudStats(manifest) {
  ...
  const available = sources.filter(
    (source) => !source.unavailable && Array.isArray(source.parts) && source.parts.length,
  );
  const lastUpdate = available.length
    ? Math.max(...available.map((source) => source.observationTime ?? manifest.fetchedAt ?? 0))
    : null;
  return {
    count: available.length,
    lastUpdate,
    error: available.length ? null : sources[0]?.reason || 'Cloud imagery unavailable',
  };
}

/** Create a GOES imagery layer managed by the application lifecycle. */
export function createCloudsLayer({ feed, cesium = Cesium, services, alpha } = {}) {
  ...
  const layer = {
    id: 'clouds', name: 'Cloud Cover', icon: '☁', source: 'NOAA GOES', updateInterval: 60000,
    init(nextViewer) { viewer = nextViewer; owned = []; enabled = false; manifest = null; guidance = null; },
    enable() { enabled = true; },
    disable() { request?.abort(); request = null; enabled = false; removeOwned(); },
    setAlpha(level) { opacity = typeof level === 'number' ? level : cloudsAlpha(level); for (const item of owned) item.alpha = opacity; },
    async update(nextViewer, { signal } = {}) {
      if (!enabled) return false;
      request?.abort();
      const controller = new AbortController();
      request = controller;
      const combined = signal && AbortSignal.any ? AbortSignal.any([signal, controller.signal]) : controller.signal;
      ...
      try {
        ...
        for (const source of nextManifest.sources) {
          ...
          for (const part of source.parts) {
            if (combined.aborted || request !== controller || !enabled || current !== generation) return false;
            try {
              const provider = await cesium.SingleTileImageryProvider.fromUrl(part.url, {
                rectangle: cesium.Rectangle.fromDegrees(part.rectangle.west, part.rectangle.south, part.rectangle.east, part.rectangle.north),
              });
              if (combined.aborted || request !== controller || !enabled || current !== generation) return false;
              const imagery = viewer.imageryLayers.addImageryProvider(provider);
              imagery.alpha = opacity;
              nextLayers.push(imagery);
            } catch (error) { console.warn('[Data:Clouds] Imagery part failed:', error); }
          }
        }
        const previous = owned; owned = nextLayers; manifest = nextManifest; guidance = null;
        for (const item of previous) viewer.imageryLayers.remove(item);
        return true;
      } catch (error) {
        if (combined.aborted || request !== controller || !enabled) return false;
