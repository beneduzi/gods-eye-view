...
export function createAprsIsSource({ fetchImpl = defaultFetch, apiUrl = '/api/aprs-live', origin = () => globalThis.location?.origin || 'http://localhost' } = {}) {
  return {
    label: 'APRS-IS',
    async getSnapshot({ maxRows = 2000 } = {}, { signal } = {}) {
      const url = new URL(apiUrl, origin()); url.searchParams.set('maxRows', String(maxRows));
      const { response, payload } = await readResponse(fetchImpl, url.toString(), { signal, cache: 'no-store' }, 'APRS-IS live');
