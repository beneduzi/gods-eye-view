...
test('reprojects RGBA imagery and splits antimeridian parts', () => {
  ...
  const result = reprojectToEquirectangular({ rgba: source, srcWidth: 64, srcHeight: 64, nav: satelliteNav(-75.2), outputHeight: 64 });
  ...
  for (const i of [0, part.width - 1, (part.height - 1) * part.width, part.height * part.width - 1]) assert.equal(part.rgba[i * 4 + 3], 0);
  const west = reprojectToEquirectangular({ rgba: source, srcWidth: 64, srcHeight: 64, nav: satelliteNav(-137), outputHeight: 64 });
  assert.equal(west.parts.length, 2);
  for (const item of west.parts) assert.ok(item.rectangle.west < item.rectangle.east && item.rectangle.west >= -180 && item.rectangle.east <= 180);
  assert.ok(west.parts.some((item) => item.rectangle.west > 0 && item.rectangle.east === 180));
  assert.ok(west.parts.some((item) => item.rectangle.west === -180 && item.rectangle.east < 0));
});
