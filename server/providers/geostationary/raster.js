...
export function reprojectToEquirectangular({
  ...
  const longitudeParts = minLon >= -180 && maxLon <= 180
    ...
    parts: longitudeParts.map(({ west, east }) => {
      ...
      for (let row = 0; row < outputHeight; row += 1) {
        for (let col = 0; col < width; col += 1) {
          ...
          const srcY = (GOES_GRID_HALF_EXTENT_RAD - scan.y) / (2 * GOES_GRID_HALF_EXTENT_RAD) * (srcHeight - 1);
          ...
          for (let channel = 0; channel < 3; channel += 1) {
            const top = rgba[(y0 * srcWidth + x0) * 4 + channel] * (1 - fx) + rgba[(y0 * srcWidth + x1) * 4 + channel] * fx;
