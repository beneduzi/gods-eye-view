...
import { GOES_SATELLITES, GOES_STAR_PRODUCT, GOES_STAR_PRODUCTS, isGoESStarProduct } from './goes/catalog.js';
...
export function infraredPalette(value) {
  const stops = [[0,[8,8,28]],[48,[20,55,150]],[96,[0,190,220]],[144,[30,190,80]],[192,[245,220,30]],[224,[240,80,10]],[255,[255,255,255]]];
  for (let i=1;i<stops.length;i++) if (value <= stops[i][0]) { const [a,ca]=stops[i-1], [b,cb]=stops[i], t=(value-a)/(b-a); return [...ca.map((v,j)=>Math.round(v+(cb[j]-v)*t)), value < 8 ? 0 : 255]; }
  return [255,255,255,255];
}
export function applyCloudProduct(rgba, product, companionGray) {
  for (let i=0;i<rgba.length;i+=4) { const v=rgba[i]; if (product === 'ABI13') rgba.set(infraredPalette(v), i); if (product === 'ABI2' && companionGray) { /* brightness is an uncalibrated proxy: hide only very warm-looking backgrounds, conservatively. */ if (companionGray[i/4] > 225) rgba[i+3] = Math.min(rgba[i+3], 32); } }
  return rgba;
}
...
export function goesProxy({ fetchImpl = fetch, now = () => Date.now(), outputHeight = null, ttlMs = 5 * 60_000, maxOutputHeight = 4096 } = {}) {
  ...
  async function refresh(product) {
    ...
    const sources = await Promise.all(GOES_SATELLITES.map(async (satellite) => {
       try {
           const frame = await fetchStarFrame(satellite, { fetchImpl, size: spec.endpointSize, product });
        ...
        if (!parts) {
          ...
          const projected = reprojectToEquirectangular({
            ...
            nav: satelliteNav(satellite.lon0),
             outputHeight: outputHeight ?? Math.min(spec.nativeHeight, maxOutputHeight),
    ...
    const result = { schemaVersion: 1, family: 'goes', product, fetchedAt, sources };
    manifests.set(product, result); return result;
  }
  ...
  async function middleware(req, res, next) {
    const url = new URL(req.url, 'http://localhost');
     ...
     if (url.pathname === '/api/goes/manifest') {
       ...
       if (!manifest || now() - manifest.fetchedAt >= ttlMs) { refreshing ||= refresh(product).finally(() => { refreshing = null; }); try { manifest = await refreshing; } catch { if (!manifest) return json(res, { schemaVersion: 1, family: 'goes', product, fetchedAt: now(), sources: [] }); } }
    ...
    if (url.pathname === '/api/goes/status') return json(res, { fetchedAt: Math.max(...[...manifests.values()].map((m) => m.fetchedAt), 0), ttlMs, sources: [...manifests.values()].flatMap((m) => m.sources).map(({ satelliteId, frameId, observationTime, unavailable, reason }) => ({ satelliteId, frameId, observationTime, unavailable, reason })) });
    return next();
  }
  return {
    name: 'goes',
